...
<p>For example, if the N-back target is <strong>1, 2, 3</strong>, then the sequence <strong>8, 2, 7</strong> is a match in position two. <strong>2, 3, 1</strong> is not a match.</p>
...
.offline-credits{margin:0 auto;padding:10px 0 22px}.offline-credits summary{cursor:pointer;font-weight:700;color:var(--blue)}
...
#training-title:focus{outline:none}
'''
...
(function () {
  ...
  function showView(moveFocus = true) {
    ...
    if (!training && !trainer.hidden) {
      ...
      const testing = feedback.textContent === 'TESTING AUDIO';
      if (typeof stop.onclick === 'function') stop.onclick();
      if (testing) feedback.textContent = '';
    }
